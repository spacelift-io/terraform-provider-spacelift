	...
	return &schema.Provider{
		Schema: map[string]*schema.Schema{
			...
			"api_token": {
				...
				ConflictsWith: []string{"api_key_endpoint", "api_key_id", "api_key_secret"},
				Optional:      true,
				Sensitive:     true,
			},
		},
...
func configureProvider(d *schema.ResourceData) (interface{}, error) {
	if err := validateProviderConfig(d); err != nil {
		return nil, err
	}

	if token, ok := d.GetOk("api_token"); ok {
		return buildClientFromToken(token.(string))
	}

	return buildClientFromAPIKeyData(d)
}

func validateProviderConfig(d *schema.ResourceData) error {
	...
	for _, config := range []string{"api_key_endpoint", "api_key_id", "api_key_secret"} {
		if _, ok := d.GetOk(config); !ok {
			acc.Push(errors.Errorf("%s must be present when the api_token is not set", config))
		}
	}
