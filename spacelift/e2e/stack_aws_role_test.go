...
func (e *StackAWSRoleTest) TestLifecycle_OK() {
	...
	e.posts(
		`{"query":"mutation($id:ID!$role_arn:String){stackSetAwsRoleDelegation(id: $id, roleArn: $roleArn){id,administrative,awsAssumedRoleARN,awsAssumeRolePolicyStatement,branch,description,name,readersSlug,repo,terraformVersion,writersSlug}}","variables":{"id":"babys-first-stack","role_arn":"arn:aws:iam::075108987694:role/terraform"}}`,
	...
	e.posts(
		`{"query":"mutation($id:ID!$role_arn:String){stackSetAwsRoleDelegation(id: $id, roleArn: $roleArn){id,administrative,awsAssumedRoleARN,awsAssumeRolePolicyStatement,branch,description,name,readersSlug,repo,terraformVersion,writersSlug}}","variables":{"id":"babys-first-stack","role_arn":null}}`,
		`{"data":{"stackSetAwsRoleDelegation":{}}}`,
		1,
	)
