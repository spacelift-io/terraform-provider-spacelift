...
- `skip_replan` (Boolean) If set to true, the apply phase will reuse the plan from the planning phase instead of re-planning. Successor of `skip_replan_when_run_all`, applies to both run-all and non-run-all stacks. Warning: this means any `mocked_outputs` referenced during planning will be applied as-is — do not enable this together with `mocked_outputs` unless you fully understand the implications, your apply may execute against mocked values rather than real ones.
